...
async def send_scheduled_message(session, bot: Bot, message: ScheduledMessage):
...
async def check_and_send_scheduled_messages(bot: Bot):
    ...
    try:
        ...
        # Свежая сессия на каждый тик: раньше здесь были rollback/remove и retry-цикл,
        # которые лечили общий scoped_session, отравленный веб-слоем.
        async with session_scope() as session:
            active_messages = (await session.execute(
                    ...
                    if message.repeat_type == RepeatType.ONCE:
                        if message.scheduled_time and message.scheduled_time <= now:
                            ...
                            if not message.last_sent_at:
                                ...
                                success = await send_scheduled_message(session, bot, message)
                                if success:
                                    sent_count += 1
